...
namespace RAWSimO.Core.Control
{
    ...
    public abstract partial class BotManager
    {
        ...
        internal List<ExtractRequest> GetPossibleRequestsofMP(Pod pod, OutputStation station, PodSelectionExtractRequestFilteringMode filterForReservation)
        {
            // Match fitting items with requests
            List<ExtractRequest> requestsToHandle = new List<ExtractRequest>();
            if (Instance.ResourceManager.NumofZiops[station] != Instance.ResourceManager.GetExtractRequestsOfStation(station).Count())
            ...
            Instance.ResourceManager.NumofZiops[station] -= j;
            if (Instance.ResourceManager.NumofZiops[station] != Instance.ResourceManager.GetExtractRequestsOfStation(station).Count() - j)
                throw new InvalidOperationException("Could not any request from the selected pod!");
            if (j != requestsToHandle.Count)
                throw new InvalidOperationException("Could not any request from the selected pod!");
            // Return the result
            return requestsToHandle;
        }
        ...
        protected bool DoExtractTaskForStation2(Bot bot, OutputStation oStation, bool extendSearch)
        {
            ...
            if (!_onTheFlyExtractSituationInvestigated || !_onTheFlyStoreSituationInvestigated)
            {
                    ...
                    }))
                {
                    ...
                    if (config.OnTheFlyExtract && bot.CurrentTask is ExtractTask)
                    {
                        ...
                        List<ExtractRequest> itemsToHandle = GetPossibleRequestsofMP(bot.Pod, extractTask.OutputStation, config.FilterForReservation);
                        if (itemsToHandle.Count == 0)
                            throw new InvalidOperationException("Could not any request from the selected pod!");
                        // Add new matches to current task
                        foreach (var item in itemsToHandle)
                            extractTask.AddRequest(item);
                        if (Instance.ResourceManager.NumofZiops[extractTask.OutputStation] != Instance.ResourceManager.GetExtractRequestsOfStation(extractTask.OutputStation).Count())
                            throw new InvalidOperationException("Could not any request from the selected pod!");
